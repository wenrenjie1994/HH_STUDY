...
import com.longyx.study.hrs.hr4.manager.impl.EmployeeManagerImpl;
...
public class EmployeeController {
    ...
    static {
        try {
            Class<?> clazz = Class.forName("com.longyx.study.hrs.hr4.manager.impl.EmployeeManagerImpl");
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) !=null) {
                ...
                if (ADD.equals(line)) {
                    ...
                    System.out.println("请输入需要修改的员工(department_id=?,employee_name=?,gender=?,degree=?,birthday=?,telephone=?,address=?,position=?,division=?,date_joined=?,employee_id=?)： ");
                    state = MODIFY;
    ...
    private static Employee getEmployee(String line) {
        ...
        for (int i = 0; i < employeeArrays.length; i++) {
            ...
            if ("employee_id".equals(values[0])) {
                employee.setEmployeeId(Integer.valueOf(values[1]));
            }else if ("department_id".equals(values[0])) {
                /**
                 * 构造部门对象
                 * @author Mr.Longyx
     ...
     */
    private static void getEmployeeByPage(String line){
